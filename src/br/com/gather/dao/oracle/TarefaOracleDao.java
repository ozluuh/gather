...
public class TarefaOracleDao implements TarefaDao {

	private Connection conexao;
	private PreparedStatement stmt;
	private ResultSet resultado;
	private ReuniaoOracleDao reuniaoOracleDao;
	private FuncionarioOracleDao funcionarioOracleDao;

	public TarefaOracleDao(Connection conexao) throws ClassNotFoundException, SQLException {
		this.conexao = conexao;
		reuniaoOracleDao = new ReuniaoOracleDao(conexao);
		funcionarioOracleDao = new FuncionarioOracleDao(conexao);
	}
	...
	private Tarefa parse(ResultSet rs) throws SQLException {
		...
		if (codigoFuncionario != 0) {
			funcionario = funcionarioOracleDao.pesquisar(codigoFuncionario);
		}

		if (codigoReuniao != 0) {
			reuniao = reuniaoOracleDao.pesquisar(codigoReuniao);
		}
	...
	public void cadastrar(Tarefa tarefa) throws SQLException {
		String sql = "INSERT INTO T_GTR_TAREFA(ID_TAREFA, ID_REUNIAO, ID_FUNCIONARIO, DS_TAREFA, OB_TAREFA, IN_STATUS, DT_CRIACAO)"
				+ " VALUES(SQ_GTR_TAREFA.NEXTVAL,?,?,?,?,?,SYSDATE)";
	...
	public void atualizar(Tarefa tarefa) throws SQLException {
		String sql = "UPDATE T_GTR_TAREFA SET " + "DS_TAREFA = ?, IN_STATUS = ?, DT_ATUALIZACAO = SYSDATE"
	...
	public void deletar(int id) throws SQLException {
		String sql = "UPDATE T_GTR_TAREFA SET DT_EXCLUSAO = SYSDATE WHERE ID_TAREFA = ?";
