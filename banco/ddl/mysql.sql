CREATE DATABASE IF NOT EXISTS `gather` DEFAULT CHARACTER SET utf8;
USE `gather`;

CREATE TABLE IF NOT EXISTS `gather`.`t_gtr_departamento` (
...
CREATE TABLE IF NOT EXISTS `gather`.`t_gtr_funcionario` (
...
CREATE TABLE IF NOT EXISTS `gather`.`t_gtr_reuniao` (
  ...
  CONSTRAINT `t_gtr_reuniao_CHECK` CHECK ((`in_status` in ('C','P')))
);

CREATE TABLE IF NOT EXISTS `gather`.`t_gtr_tarefa` (
  `id_tarefa` int NOT NULL AUTO_INCREMENT,
  `id_funcionario` int NOT NULL,
  `id_reuniao` int NOT NULL,
  `ds_tarefa` varchar(120) NOT NULL,
  `ob_tarefa` varchar(400) DEFAULT NULL,
  `in_status` char(1) NOT NULL DEFAULT 'P',
  `dt_criacao` datetime NOT NULL,
  `dt_atualizacao` datetime DEFAULT NULL,
  `dt_exclusao` datetime DEFAULT NULL,
  ...
  CONSTRAINT `t_gtr_tarefa_CHECK` CHECK ((`in_status` in ('C','P')))
);
